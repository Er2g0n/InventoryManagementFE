...
import { DndContext, KeyboardSensor, MouseSensor, TouchSensor, closestCenter, DragEndEvent, useSensor, useSensors } from "@dnd-kit/core";
...
function Filter ({ column }: { column: any }) {
  ...
  return filterVariant === "select" ? (
          ...
          <div className="flex flex-col p-1">
            {sortedUniqueValues.map((value: any) => (
              <Radio
                ...
                value={value}
              >
                {column.columnDef?.cell && column.columnDef.cell({ getValue: () => value })}
              </Radio>
            ))}
          </div>
        </Card>
    ...
    >
      <div
        className={`cursor-pointer ${sortedUniqueValues.includes(columnFilterValue) ? "text-blue-600" : ""}`}
      >
        <FilterFilled />
      </div>
    </Dropdown>
...
const DragAlongCell = ({ cell }: { cell: any }) => {
  ...
  return (
    <td
      ref={setNodeRef}
      style={style}
      className={`border border-gray-300 p-2 relative ${isDragging ? "opacity-80 z-10" : "opacity-100 z-0"}`}
    >

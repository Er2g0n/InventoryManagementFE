...
const ListGoodsReceiptNote: React.FC<ListGoodsReceiptNoteProps> = React.memo(({ onEdit, refreshTrigger }) => {
    const { GoodsReceiptNotes, GoodsReceiptNoteLines, loading, error, loadGoodsReceiptNote, loadGoodsReceiptNoteLine, ClearGoodsReceiptNoteLine, deleteGoodsReceiptNote } = useGoodsReceiptNote();
    const [globalFilter, setGlobalFilter] = useState('');
    const [pageIndex, setPageIndex] = useState(0);
    const [pageSize, setPageSize] = useState(10);
    const [isFunction, setIsFunction] = useState(false);
    const [currentPageIndex, setcurrentPageIndex] = useState(0);
    const [selectedGrnCode, setSelectedGrnCode] = useState<GoodsReceiptNote | null>(null);
    ...
    const LoadGoodsReceiptNoteDetail = async (GoodsReceiptNote: GoodsReceiptNote) => {
        try {
            setLoading2(true);
            loadGoodsReceiptNoteLine(GoodsReceiptNote.grnCode);
            setSelectedGrnCode(GoodsReceiptNote)
        } catch (error) {
            ...
            cell: ({ row }) => (
                <Space>
                    ...
                    <Button
                        ...
                        onClick={() => LoadGoodsReceiptNoteDetail(row.original)}

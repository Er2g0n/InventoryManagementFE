...
const ListColor: React.FC<ListColorProps> = React.memo(({ onEdit }) => {
    ...
    const uniqueColorCodes = [...new Set(colors.map(c => c.colorCode))];
    const uniqueColorNames = [...new Set(colors.map(c => c.colorName))];
    ...
    const showDeleteConfirm = (record: Color) => {
        Modal.confirm({
            title: (
                <div>

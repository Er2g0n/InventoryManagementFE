...
interface ListUnitOfMeasureProps {
  onEdit: (unitOfMeasure: UnitOfMeasure) => void;
}

const ListUnitOfMeasure: React.FC<ListUnitOfMeasureProps> = React.memo(({ onEdit }) => {
  const { uoMList, loading, error, loadUoM, deleteUoM } = useUoM();
  ...
  const handleDelete = (uoMCode: string) => {
    deleteUoM(uoMCode);
  };

  const uniqueUoMCodes = [...new Set(uoMList.map(uom => uom.uoMCode))];
  const uniqueUoMNames = [...new Set(uoMList.map(uom => uom.uoMName))];

  const columns: ColumnDef<UnitOfMeasure>[] = [
    {
      header: ({ column }) => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <span style={{ fontWeight: 600 }}>Unit Code</span>
          <select
  ...
  const table = useReactTable<UnitOfMeasure>({
    data: uoMList,
    columns,
    state: {
      sorting,
  ...
  const showDeleteConfirm = (record: UnitOfMeasure) => {
    Modal.confirm({
      title: (
        <div>
          <div>Bạn có chắc chắn muốn xóa:</div>
          <div style={{ fontWeight: 600, marginTop: 4 }}>
            {record.uoMCode} - {record.uoMName}
          </div>
        </div>
      ),
      ...
      cancelText: 'Hủy',
      onOk() {
        handleDelete(record.uoMCode);
      },
    });
  };

  if (loading && uoMList.length === 0) {
